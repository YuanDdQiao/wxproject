...
module.exports={
  ...
  getComment: async ctx => {
    ...
    if(!isNaN(commentid)){
      ctx.state.data = (await DB.query("select * from mov_comment where movie_id = ? limit 1;", [commentid]))[0]
    }else{
      ctx.state.data = {}
    }
  },
  ...
  commentsListItem: async ctx => {
    let commentId = + ctx.request.query.comment_id
    if (!isNaN(commentId)) {
      ctx.state.data = await DB.query("select * from mov_comment where movie_id = ?", [commentId])
    }else{
      ctx.state.data = []
    }
  }
